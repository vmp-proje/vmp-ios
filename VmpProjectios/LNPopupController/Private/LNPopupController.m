...
{
  [[NSNotificationCenter defaultCenter] postNotificationName:@"MusicPlayerFullScreenOpenedNotification" object:self];
...
- (void)_repositionPopupCloseButton
{
  ...
  _popupCloseButtonTopConstraint.constant = _popupContentView.popupCloseButton.style == LNPopupCloseButtonStyleRound ? 30 : 20;
    ...
    if(possibleBar)
    {
        if (_popupContentView.popupCloseButtonAutomaticallyUnobstructsTopBars)
            ...
            _popupCloseButtonTopConstraint.constant += 35;
    }
...
//    _popupContentView.popupCloseButton.heightAnchor.
		_popupCloseButtonTopConstraint = [_popupContentView.popupCloseButton.topAnchor constraintEqualToAnchor:_popupContentView.contentView.topAnchor constant:buttonStyle == LNPopupCloseButtonStyleRound ? 30 : 8];
...
//      _popupContentView.contentView.alpha = 0.5
      [_popupContentView.popupCloseButton.topAnchor constraintEqualToAnchor:_popupContentView.contentView.topAnchor constant:buttonStyle == LNPopupCloseButtonStyleRound ? 25 + topPadding : 8].active = TRUE;
      
      [_popupContentView.popupCloseButton.leadingAnchor constraintEqualToAnchor:_popupContentView.contentView.leadingAnchor constant:24].active = YES;

      [_popupContentView.popupCloseButton.widthAnchor constraintEqualToConstant:52.0].active
      = YES;
      [_popupContentView.popupCloseButton.heightAnchor constraintEqualToConstant:52.0].active = YES;
      
      [NSLayoutConstraint activateConstraints:@[_popupCloseButtonTopConstraint]];
      //      _popupCloseButtonHorizontalConstraint = [_popupContentView.popupCloseButton.leadingAnchor constraintEqualToAnchor:_popupContentView.contentView.leadingAnchor constant:24];
      return;
		}
		else
		{
...
- (void)presentPopupBarAnimated:(BOOL)animated openPopup:(BOOL)open completion:(void(^)(void))completionBlock
{
	...
	if(_popupControllerTargetState == LNPopupPresentationStateHidden)
	{
		...
		[UIView animateWithDuration:animated ? 0.25 : 0.0 delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseInOut animations:^ {
...
- (void)dismissPopupBarAnimated:(BOOL)animated duration:(CGFloat)duration completion:(void (^)(void))completionBlock
{
  if(_popupControllerState != LNPopupPresentationStateHidden)
  {
    void (^dismissalAnimationCompletionBlock)(void) = ^
    {
      ...
      [UIView animateWithDuration:animated ? duration : duration delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseInOut animations:^
       {
         CGRect barFrame = self.popupBar.frame;
...
- (void)dismissPopupBarAnimated:(BOOL)animated completion:(void(^)(void))completionBlock
{
	if(_popupControllerState != LNPopupPresentationStateHidden)
	{
		void (^dismissalAnimationCompletionBlock)(void) = ^
		{
			...
			[UIView animateWithDuration:animated ? 0 : 0.0 delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseInOut animations:^
			 {
				 CGRect barFrame = self.popupBar.frame;
				 barFrame.size.height = 0;
				 self.popupBar.frame = barFrame;
